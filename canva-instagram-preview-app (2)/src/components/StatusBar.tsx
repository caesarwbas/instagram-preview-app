...
interface StatusBarProps {
  /** When true, renders as a transparent absolute overlay (for story/reel) */
  dark?: boolean;
  device?: DeviceType;
  theme?: ThemeMode;
}

const StatusBar: React.FC<StatusBarProps> = ({ dark = false, device = 'ios', theme: _theme = 'light' }) => {
  void _theme; // Theme colors handled by parent .phone-screen.dark-theme CSS
  const now = new Date();
  const time = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false });

  // Only apply the overlay class when explicitly told to (story/reel).
  // In feed mode, dark=false — the theme color is handled via parent
  // .phone-screen.dark-theme CSS rules instead.
  const overlayClass = dark ? 'status-bar-dark' : '';

  if (device === 'android') {
    return (
      <div className={`status-bar status-bar-android ${overlayClass}`}>
  ...
  return (
    <div className={`status-bar status-bar-ios ${overlayClass}`}>
