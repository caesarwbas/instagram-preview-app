...
const InstagramPreview: React.FC<InstagramPreviewProps> = ({ postData, onSlideChange }) => {
  ...
  return (
    <div className={`phone-frame ${isIos ? 'ios' : 'android'}`} style={{ margin: '0 auto' }}>
      {/* Dynamic Island / Punch Hole - these sit on top of the frame, outside the screen flow */}
      {isIos ? (
      ...
      {/* Phone Screen */}
      <div
        className={`phone-screen ${isDark ? 'dark-theme' : 'light-theme'} ${isIos ? 'ios-device' : 'android-device'}`}
        style={{
          height: isStoryOrReel ? 708 : undefined,
          maxHeight: 750,
          display: 'flex',
      ...
      >
        {previewType === 'story' ? (
          ...
          <>
            {/* STATUS BAR - fixed at top, never scrolls, highest z-index */}
            <div className="feed-status-bar-wrapper" style={{
              flexShrink: 0,
              position: 'relative',
              zIndex: 100,
            }}>
            ...
            {/* INSTAGRAM APP HEADER - fixed below status bar */}
            <div className="feed-ig-header-wrapper" style={{
              flexShrink: 0,
              position: 'relative',
              zIndex: 90,
            }}>
            ...
            <div className="feed-content-wrapper" style={{
              flex: 1,
              overflowY: 'auto',
              overflowX: 'hidden',
              position: 'relative',
